...
export const SignUpPage: React.FC<SignUpPageProps> = ({ onComplete, initialEmail = '', initialPassword = '', initialUsername = '' }) => {
  const { signUp, theme, toggleTheme } = useUser();
  const [userType, setUserType] = useState<'tourist' | 'local'>('tourist');  const [step, setStep] = useState(1); // Start at 1 now since auth is handled in wrapper
...
  };
  ...
  const canProceedStep1 = email && password && username;
  const canProceedStep2 = name && age;
  const canProceedStep3 = country && language && currency && appLanguage;
